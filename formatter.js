...
function toWorkbook(worksheets, styles) {
    ...
    for (let i = 0; i < worksheets.length; i++) {
        ...
        let style = styles[i];
        for (let addr in style) {
            if ('fill' in style[addr]) {
                xlsx.utils.set_fill(workbook, worksheet['worksheet'], cell(addr), style[addr]['fill']);
            }
            if ('border' in style[addr]) {
                xlsx.utils.set_border(workbook, worksheet['worksheet'], cell(addr), style[addr]['border']);
            }
        }
    }
